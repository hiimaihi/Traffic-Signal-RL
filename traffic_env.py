"""
交通信号灯自适应控制 — 环境定义与物理逻辑仿真
================================================
基于 Gymnasium 的单十字路口交通流仿真环境。

核心特性:
  - 泊松分布车辆到达 (四方向独立可配)
  - 两相位控制 (南北/东西)
  - 可变状态空间: S4(4维)/S8(8维)/S9(9维)
  - 多种奖励函数: R1(仅排队)/R2(排队+切换)/R3(排队+等待)
  - 5种交通模式: uniform / peak_hour / tidal / burst / low_traffic
  - 潮汐车流模式 (早高峰 / 平峰 / 动态扰动)
  - 黄灯过渡机制
  - 环境配置注册表 (env_config)

Author: RL Traffic Signal Control Project
"""
